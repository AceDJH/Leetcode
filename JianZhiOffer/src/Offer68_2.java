...
public class Offer68_2 {
    public static void main(String[] args) {
        ...
        node4.left = node7;
        node4.right = node8;
        System.out.println(new Solution68_2().lowestCommonAncestor(root, node1, node8).val);
    }
}
...
class Solution68_2 {
    public TreeNode lowestCommonAncestor(TreeNode root, TreeNode p, TreeNode q) {
        ...
        boolean pFind = findPath(root, p, pPath);
        boolean qFind = findPath(root, q, qPath);
        if (pFind && qFind){
    ...
    public boolean findPath(TreeNode root, TreeNode node, List<TreeNode> list){
        ...
        boolean isFind = findPath(root.left, node, list) || findPath(root.right, node, list);
        if (!isFind){
